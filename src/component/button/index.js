...
const baseButton = (className, props, type) => {
    const cName = props.className ? `${className} ${props.className}` : `${className}`
    const changeableProps = { ...props }
    const { active, onClick } = changeableProps
    delete changeableProps.className

    if (active) {
        delete changeableProps.active

        function active(e) {
            const buttons = window.document.querySelectorAll(`.${className}`)
        ...
        if (onClick) {
            changeableProps.onClick = function (e) {
                active(e)
                onClick(e)
            }
        } else {
            changeableProps.onClick = active
        }

    }
    switch (type) {
        case 'button':
            return <button className={cName} {...changeableProps}>{props.children}</button>
        case 'link':
            return <Link className={cName}  {...changeableProps}>{props.children}</Link>
    }
}

const ButtonPrimary = (props) => {
    return baseButton('primary', props, 'button')
}

const ButtonSecondary = (props) => {
    return baseButton('secondary', props, 'button')
}

const ButtonDanger = (props) => {
    return baseButton('danger', props, 'button')
}


const LinkPrimary = (props) => {
    return baseButton('primary', props, 'link')
}
